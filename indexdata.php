...
{
 $sql_query="DELETE FROM data WHERE id=".$_GET['delete_id'];
 mysqli_query($con,$sql_query);
 header("Location: $_SERVER[PHP_SELF]");
}
if(isset($_GET['changestatus_id']))
{
 $sql_query="UPDATE data SET `status`='".$_GET['status']."' WHERE id=".$_GET['changestatus_id'];
 mysqli_query($con,$sql_query);
 header("Location: $_SERVER[PHP_SELF]");
}
?>
...
function view_id(id)
{
  window.location.href='view_data.php?view_id='+id;
}
function delete_id(id)
{
 if(confirm('Sure to Delete ?'))
 {
  window.location.href='indexdata.php?delete_id='+id;
 }
}
function changestatus_id(id,status)
{
  window.location.href='indexdata.php?changestatus_id='+id+'&status='+status;
}
</script>
</head>
<body>
...
<div id="container">
 <div id="table-responsive">
    ...
    <tr>
    <th colspan="5"><a href="add_data.php">add data.</a></th>
    </tr>
    <th>SL NO</th>
    <th>id</th>
